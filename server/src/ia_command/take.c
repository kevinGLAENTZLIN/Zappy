...
static void handle_take_response(server_t *server, int i, char *object)
{
    if (strcmp(object, "food") == 0)
        send_to_all_gui(server, "pgt #%d %d", PLAYER->id, 0);
    if (strcmp(object, "linemate") == 0)
        send_to_all_gui(server, "pgt #%d %d", PLAYER->id, 1);
    if (strcmp(object, "deraumere") == 0)
        send_to_all_gui(server, "pgt #%d %d", PLAYER->id, 2);
    if (strcmp(object, "sibur") == 0)
        send_to_all_gui(server, "pgt #%d %d", PLAYER->id, 3);
    if (strcmp(object, "mendiane") == 0)
        send_to_all_gui(server, "pgt #%d %d", PLAYER->id, 4);
